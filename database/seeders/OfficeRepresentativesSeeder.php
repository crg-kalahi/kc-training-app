...
class OfficeRepresentativesSeeder extends Seeder
{
    public function run()
    {
        ...
        for ($i = 1; $i <= 20; $i++) {
            $data[] = [
                'title' => $faker->jobTitle,  // Generate a random job title
                'order' => $i,                // Set the order field as the loop index
                'is_default' => $i % 2 == 0 ? true : false,  // Alternate between true and false for is_default
                'deleted_at' => $faker->boolean(20) ? Carbon::now()->subDays(rand(1, 30)) : null, // 20% chance for deleted_at
        ...
        DB::table('office_representatives')->insert($data);
    }
}
